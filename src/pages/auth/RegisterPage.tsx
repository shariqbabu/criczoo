...
export default function RegisterPage() {
  const { signUp: register, signInWithGoogle: loginWithGoogle } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await register(email, password, displayName, displayName.toLowerCase().replace(/\s+/g, '_'));
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 to-emerald-100 px-4 py-8">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-muted-foreground mt-1">Join the cricket community</p>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 border border-border py-2.5 rounded-lg font-medium hover:bg-muted transition-colors disabled:opacity-50"
          >
