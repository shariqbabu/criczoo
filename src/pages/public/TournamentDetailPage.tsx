      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-gradient-to-r from-yellow-500 to-amber-500 text-white rounded-2xl p-8 mb-6">
          ...
          <div className="flex flex-wrap gap-4 text-sm text-yellow-100">
            <span className="flex items-center gap-1"><Calendar className="w-4 h-4" /> {tournament.startDate ? new Date(tournament.startDate).toLocaleDateString() : 'TBD'}</span>
            <span className="flex items-center gap-1"><Users className="w-4 h-4" /> {tournament.teamCount || 0} teams</span>
          </div>
        </div>
          ...
          {matches.length === 0 ? (
            ...
            <div className="space-y-3">
              {matches.map(match => (
                ...
                >
                  <div>
                    <p className="font-medium text-sm">{match.team1Name} vs {match.team2Name}</p>
